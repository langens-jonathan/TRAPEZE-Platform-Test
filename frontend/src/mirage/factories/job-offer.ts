...
export const JobOfferFactory = Factory.extend({
  title() { return `${jobs[Math.floor(Math.random() * jobs.length)]} in ${faker.address.cityName()}`},
  createdOn() { return faker.date.recent(60)},
  description() { return faker.lorem.paragraph(5)},
});

export default JobOfferFactory
